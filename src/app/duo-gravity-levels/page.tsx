...
interface DuoGravityLevelWithRecord extends DuoGravityLevel {
  bestTime?: number | null;
  bestSteps?: number | null;
  isOfficial?: boolean;
}

interface UserDuoGravityLevel {
  id: string;
  name: string;
  verified: boolean;
  published: boolean;
...
export default function DuoGravityLevelsPage() {
  const router = useRouter();
  const { data: session, status } = useSession();
  const [levels, setLevels] = useState<DuoGravityLevelWithRecord[]>([]);
  const [userLevels, setUserLevels] = useState<UserDuoGravityLevel[]>([]);
    ...
    const fetchData = async () => {
      try {
        ...
        const [levelsRes, userLevelsRes] = await Promise.all([
          fetch('/api/duo-gravity-levels'),
          fetch('/api/user-duo-gravity-levels'),
        ]);
  ...
  const handleDeleteUserLevel = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/user-duo-gravity-levels/${id}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
  ...
  const handleEditUserLevel = (id: string) => {
    router.push(`/duo-gravity-editor?edit=${id}`);
  };
  ...
  const handlePlayOfficialLevel = (id: number | string) => {
    router.push(`/duo-gravity-game/${id}`);
  };
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
          <div>
            ...
            <p className="text-slate-400">重力解谜，协作攀登</p>
          </div>
        ...
        <div>
          ...
          {userLevels.length === 0 ? (
            <div className="glass-card p-12 text-center">
              <MoveDown className="w-12 h-12 text-slate-500 mx-auto mb-4" />
              <p className="text-slate-400 mb-4">还没有创建任何重力关卡</p>
              <Link
                href="/duo-gravity-editor"
                className="btn-primary inline-flex items-center gap-2"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {userLevels.map((level) => (
                <div
                  key={level.id}
                  className="glass-card gradient-border p-6 transition-all duration-300 hover:scale-[1.02] hover:shadow-xl hover:shadow-purple-500/20"
                >
