import { PokemonType, PokemonData, BaseStats, Stats, MoveData, MoveCategory } from '../types';
import { getMove } from './moves';

export const PokemonDataList: PokemonData[] = [
    {
        id: 1,
        name: '皮卡丘',
...
export class Pokemon {
    ...
    constructor(pokemonData: PokemonData, level: number = 50) {
...
export function createRandomPokemon(level: number = 50): Pokemon {
    const randomIndex = Math.floor(Math.random() * PokemonDataList.length);
    return createPokemon(PokemonDataList[randomIndex].id, level);
}

export function getAllPokemonData(): PokemonData[] {
