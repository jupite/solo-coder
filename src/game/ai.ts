import { PokemonType, MoveData } from '../types';
import { Pokemon } from '../pokemon/pokemon';
import { getTypeEffectiveness, getStab } from '../types';
...
export class PokemonAI {
    ...
    selectMove(aiPokemon: Pokemon, playerPokemon: Pokemon): MoveData | null {
    ...
    private selectRandomMove(moves: MoveData[]): MoveData {
        const randomIndex = Math.floor(Math.random() * moves.length);
        return moves[randomIndex];
    }

    private selectSemiSmartMove(moves: MoveData[], aiPokemon: Pokemon, playerPokemon: Pokemon): MoveData {
    ...
    private selectSmartMove(moves: MoveData[], aiPokemon: Pokemon, playerPokemon: Pokemon): MoveData {
        const moveScores = moves.map(move => {
            let score = 0;
    ...
    estimateDamage(move: MoveData, attacker: Pokemon, defender: Pokemon): number {
