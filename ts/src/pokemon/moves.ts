import type { MoveData, PokemonType, MoveCategory } from '../types';

export interface PokemonMoveData extends MoveData {
  id: string;
}

export const PokemonMoves: Record<string, MoveData> = {
  tackle: {
    name: '撞击',
    type: 'normal',
    power: 40,
    accuracy: 100,
    pp: 35,
    category: 'physical',
    description: '用整个身体撞击对手。'
  },
  scratch: {
    name: '抓挠',
    type: 'normal',
    power: 40,
    accuracy: 100,
    pp: 35,
    category: 'physical',
    description: '用锋利的爪子抓挠对手。'
  },
  ember: {
    name: '火花',
    type: 'fire',
    power: 40,
    accuracy: 100,
    pp: 25,
    category: 'special',
    description: '喷出小火焰攻击对手。'
  },
  flamethrower: {
    name: '喷射火焰',
    type: 'fire',
    power: 90,
    accuracy: 100,
    pp: 15,
    category: 'special',
    description: '喷出强烈的火焰攻击对手。'
  },
  fireBlast: {
    name: '大字爆炎',
    type: 'fire',
    power: 110,
    accuracy: 85,
    pp: 5,
    category: 'special',
    description: '用大字形的火焰攻击对手。'
  },
  waterGun: {
    name: '水枪',
    type: 'water',
    power: 40,
    accuracy: 100,
    pp: 25,
    category: 'special',
    description: '喷射水流攻击对手。'
  },
  hydroPump: {
    name: '水炮',
    type: 'water',
    power: 110,
    accuracy: 80,
    pp: 5,
    category: 'special',
    description: '用高压水柱攻击对手。'
  },
  surf: {
    name: '冲浪',
    type: 'water',
    power: 90,
    accuracy: 100,
    pp: 15,
    category: 'special',
    description: '用巨大的波浪攻击对手。'
  },
  vineWhip: {
    name: '藤鞭',
    type: 'grass',
    power: 45,
    accuracy: 100,
    pp: 25,
    category: 'physical',
    description: '用藤蔓抽打对手。'
  },
  razorLeaf: {
    name: '飞叶快刀',
    type: 'grass',
    power: 55,
    accuracy: 95,
    pp: 25,
    category: 'physical',
    description: '用锋利的叶子切割对手。'
  },
  solarBeam: {
    name: '阳光烈焰',
    type: 'grass',
    power: 120,
    accuracy: 100,
    pp: 10,
    category: 'special',
    description: '聚集阳光发射强烈光束。'
  },
  thunderShock: {
    name: '电击',
    type: 'electric',
    power: 40,
    accuracy: 100,
    pp: 30,
    category: 'special',
    description: '用电流攻击对手。'
  },
  thunderbolt: {
    name: '十万伏特',
    type: 'electric',
    power: 90,
    accuracy: 100,
    pp: 15,
    category: 'special',
    description: '用强烈的电击攻击对手。'
  },
  quickAttack: {
    name: '电光一闪',
    type: 'normal',
    power: 40,
    accuracy: 100,
    pp: 30,
    category: 'physical',
    description: '以极快的速度攻击对手。'
  },
  ironTail: {
    name: '铁尾',
    type: 'steel',
    power: 100,
    accuracy: 75,
    pp: 15,
    category: 'physical',
    description: '用坚硬的尾巴攻击对手。'
  },
  bite: {
    name: '咬碎',
    type: 'normal',
    power: 60,
    accuracy: 100,
    pp: 25,
    category: 'physical',
    description: '用锋利的牙齿撕咬对手。'
  },
  dragonRage: {
    name: '龙之怒',
    type: 'dragon',
    power: 40,
    accuracy: 100,
    pp: 10,
    category: 'special',
    description: '释放龙的怒火。'
  },
  dragonClaw: {
    name: '龙爪',
    type: 'dragon',
    power: 80,
    accuracy: 100,
    pp: 15,
    category: 'physical',
    description: '用锋利的龙爪攻击对手。'
  },
  psychic: {
    name: '精神强念',
    type: 'psychic',
    power: 90,
    accuracy: 100,
    pp: 10,
    category: 'special',
    description: '用强大的精神力攻击对手。'
  },
  confusion: {
    name: '念力',
    type: 'psychic',
    power: 50,
    accuracy: 100,
    pp: 25,
    category: 'special',
    description: '用较弱的精神力攻击对手。'
  },
  shadowBall: {
    name: '影子球',
    type: 'ghost',
    power: 80,
    accuracy: 100,
    pp: 15,
    category: 'special',
    description: '投掷一团黑影攻击对手。'
  },
  lick: {
    name: '舌舔',
    type: 'ghost',
    power: 30,
    accuracy: 100,
    pp: 30,
    category: 'physical',
    description: '用舌头舔舐对手。'
  },
  rockThrow: {
    name: '落石',
    type: 'rock',
    power: 50,
    accuracy: 90,
    pp: 15,
    category: 'physical',
    description: '投掷岩石攻击对手。'
  },
  rockSlide: {
    name: '岩崩',
    type: 'rock',
    power: 75,
    accuracy: 90,
    pp: 10,
    category: 'physical',
    description: '用大量岩石攻击对手。'
  },
  iceBeam: {
    name: '冰冻光束',
    type: 'ice',
    power: 90,
    accuracy: 100,
    pp: 10,
    category: 'special',
    description: '发射冰冻光线攻击对手。'
  },
  powderSnow: {
    name: '细雪',
    type: 'ice',
    power: 40,
    accuracy: 100,
    pp: 25,
    category: 'special',
    description: '用冰冷的雪攻击对手。'
  },
  wingAttack: {
    name: '翅膀攻击',
    type: 'flying',
    power: 60,
    accuracy: 100,
    pp: 35,
    category: 'physical',
    description: '用翅膀攻击对手。'
  },
  thunder: {
    name: '打雷',
    type: 'electric',
    power: 110,
    accuracy: 70,
    pp: 10,
    category: 'special',
    description: '用强烈的雷电攻击对手。'
  },
  sing: {
    name: '唱歌',
    type: 'normal',
    power: 0,
    accuracy: 55,
    pp: 15,
    category: 'status',
    description: '用悦耳的歌声让对手入睡。'
  },
  moonblast: {
    name: '月亮之力',
    type: 'fairy',
    power: 95,
    accuracy: 100,
    pp: 15,
    category: 'special',
    description: '借助月亮的力量攻击对手。'
  },
  playRough: {
    name: '嬉闹',
    type: 'fairy',
    power: 90,
    accuracy: 90,
    pp: 10,
    category: 'physical',
    description: '与对手嬉闹并攻击。'
  },
  supersonic: {
    name: '超音波',
    type: 'normal',
    power: 0,
    accuracy: 55,
    pp: 20,
    category: 'status',
    description: '发出特殊的声波让对手混乱。'
  },
  poisonFang: {
    name: '毒牙',
    type: 'poison',
    power: 50,
    accuracy: 100,
    pp: 15,
    category: 'physical',
    description: '用毒牙撕咬对手。'
  },
  bubble: {
    name: '泡泡',
    type: 'water',
    power: 40,
    accuracy: 100,
    pp: 30,
    category: 'special',
    description: '喷出无数泡泡攻击对手。'
  },
  hypnosis: {
    name: '催眠术',
    type: 'psychic',
    power: 0,
    accuracy: 60,
    pp: 20,
    category: 'status',
    description: '施加催眠让对手入睡。'
  },
  doubleSlap: {
    name: '连环巴掌',
    type: 'normal',
    power: 15,
    accuracy: 85,
    pp: 10,
    category: 'physical',
    description: '用巴掌连续拍打对手。'
  }
};

export function getMove(moveId: string): MoveData | null {
  return PokemonMoves[moveId] || null;
}