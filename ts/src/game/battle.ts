import type { Pokemon, Item, BattleState, TeamPokemon, ActivePokemonState, DamageResult, MoveResult, SwitchResult, ItemResult, PlayerBuffs } from '../types';
import { createPokemon } from '../pokemon/pokemon';
import { getTypeEffectiveness, getStab } from '../pokemon/types';
import { PokemonAI, AIDifficulty } from './ai';
...
export class BattleSystem {
  ...
  constructor(playerPokemonIds: number[], enemyPokemonIds: number[], ai?: PokemonAI, playerBackpack: Record<string, Item> = {}) {
  ...
  calculateDamage(move: { power: number; accuracy: number; type: string; category: string }, attacker: Pokemon, defender: Pokemon, isPlayer: boolean): DamageResult {
    ...
    const stab = getStab(move.type as any, attacker.types);

    let totalEffectiveness = 1;
    for (const defenderType of defender.types) {
      totalEffectiveness *= getTypeEffectiveness(move.type as any, defenderType);
    }
  ...
  executeMove(move: { name: string; power: number; type: string; accuracy: number; category: string }, attacker: Pokemon, defender: Pokemon, isPlayer: boolean): MoveResult {
    ...
    return { message, ...result, move: move as any };
  }

  getAvailablePokemonIndices(team: Pokemon[]): number[] {
    return team
  ...
  canSwitchPokemon(isPlayer: boolean): boolean {
  ...
  switchPokemon(newIndex: number, isPlayer: boolean): SwitchResult {
    const team = isPlayer ? this.playerTeam : this.enemyTeam;
    const activeIndex = isPlayer ? this.activePlayerIndex : this.activeEnemyIndex;
  ...
  checkTeamFainted(team: Pokemon[]): boolean {
  ...
  useItem(itemId: string): ItemResult {
    ...
    const result: ItemResult = { success: true, message, item };
  ...
  enemyTurn(): MoveResult | { message: string; damage: number; isSwitch?: boolean } | null {
    if (this.battleOver || this.isPlayerTurn) {
      return null;
    }
  ...
  getBattleState(): BattleState {
    const playerTeamState: TeamPokemon[] = this.playerTeam.map((pokemon, index) => ({
      id: pokemon.id,
      name: pokemon.name,
    ...
    const enemyTeamState: TeamPokemon[] = this.enemyTeam.map((pokemon, index) => ({
      id: pokemon.id,
      name: pokemon.name,
      currentHp: pokemon.currentHp,
      maxHp: pokemon.maxHp,
    ...
    const playerPokemonState: ActivePokemonState = {
    ...
    const enemyPokemonState: ActivePokemonState = {
    ...
    return {
      ...
      playerPokemon: playerPokemonState,
      enemyPokemon: enemyPokemonState,
